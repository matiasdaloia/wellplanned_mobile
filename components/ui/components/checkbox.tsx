...
export const CheckboxIcon = ({ checked = false }: IconProps) => {
  const color = checked ? colors.primary.light : colors.charcoal[400];
    ...
    <CheckboxRoot checked={checked} testID={testID} {...props}>
      ...
      {label ? (
        <Label
          ...
          className={twMerge("pr-2", labelClassName)}
        />
      ) : null}
    </CheckboxRoot>
  );
...
export const SwitchIcon = ({ checked = false }: IconProps) => {
  ...
  const backgroundColor = checked ? colors.primary.light : colors.charcoal[400];
